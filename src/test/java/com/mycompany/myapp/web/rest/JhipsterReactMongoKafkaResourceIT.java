...
class JhipsterReactMongoKafkaResourceIT {
    ...
    void setup() {
        ...
        JhipsterReactMongoKafkaResource kafkaResource = new JhipsterReactMongoKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/jhipster-react-mongo-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/jhipster-react-mongo-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
